...
This section explains what each folder/file does so a new user can quickly understand how to use this project after cloning.
...
- `trainig/` - Model training script (folder name is kept as-is in this project)
...
- `process_dataset.py` - Prepares dataset into training-ready format
- `remove_duplicates.py` - Removes duplicate entries/images
...
- Label preservation: Copies YOLO label files unchanged to keep annotation alignment intact.
- Config-based paths: Uses `.env` variables (`DATASET_PATH`, `PROCESSED_DATASET_PATH`) for flexible local setup.
...
4. (Optional but recommended) Run preprocessing scripts if you are preparing your own dataset.
...
- The `runs/` directory can grow quickly; clean old runs if disk usage becomes high.